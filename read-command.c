...
int isValid(char *c)
{
    ...
    while(c[i] != '\0')
    {
        ...
        //Check that < have valid tokens on either side
        else if(c[i] == '<')
        {
           while(c[sub-1] == ' ')
             sub--;
        ...
        else if(c[i] == '>')
        {
           ...
           if(c[sub-1] == '\n' || isSpecialToken(c[sub-1]))
           {
              //If previous token is not > or <, error
              if(c[sub-1] != '>' && c[sub-1] != '<'){
                fprintf(stderr, "Parse Error in Line %d: Cannot have a special token or newline before IO redirect.\n", lineNumber);
                return 0;
              }
        ...
        else if(c[i] == '|')
        {
           ...
           if(c[sub-1] == '\n' || isSpecialToken(c[sub-1]))
           {
              //If previous token is not >, error
              if(c[sub-1] != '>'){
...
//Assign operators and pop off stack when assignment is finished
    ...
    // For NOCLOBBER OUTPUTS
      ...
      int len = 0;
      while(isOrdinaryToken(c[i])){
        cmdTemp->clobber_output[len] = c[i];
        len++;
        i++;  
      }
